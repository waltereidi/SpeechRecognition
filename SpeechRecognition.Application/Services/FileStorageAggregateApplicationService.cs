...
namespace SpeechRecognition.Application.Services
{
    public class FileStorageAggregateApplicationService : IApplicationService
    {
        ...
        private async Task HandleSaveFile(V1.UpdateFileStorage cmd)
        {
            ...
            await HandleFileUpdate( fileStorageAgg ,cmd.originalFileName , fi );
        ...
        private async Task HandleFileUpdate(FileStorageAggregate fsa, string orignalFileName ,FileInfo fi)
        {
            fsa.AddFileStorageLocal( fi, orignalFileName );
